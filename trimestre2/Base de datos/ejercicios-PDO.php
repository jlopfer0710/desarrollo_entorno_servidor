<?php 
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['eliminar'])) {
        if (!empty($_POST['eliminar_noticias'])) {
            ...
            if ($pdo->exec($queryDelete)) {
                echo "<p style='color:green;'>Noticias eliminadas correctamente.</p>";
            } else {
                echo "<p style='color:red;'>Error al eliminar noticias.</p>";
            }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['insertar'])) {
        if (!empty($_POST['titulo']) && !empty($_POST['texto']) && !empty($_POST['categoria'])) {
            ...
            if ($stmt->execute([$titulo, $texto, $categoria, $fecha, $imagenRuta])) {
                header("Location: " . $_SERVER['PHP_SELF']);
                exit();
            } else {
                echo "<p style='color:red;'>Error al insertar la noticia.</p>";
            }
        } else {
